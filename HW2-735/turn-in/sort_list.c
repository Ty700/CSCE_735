...
int ptr[MAX_THREADS+1];         // Starting positions for each sublist
...
void* thread_sort(void* arg) {
    ...
    // Sort local list using qsort
    my_list_size = ptr[my_id+1] - ptr[my_id];
    qsort(&list[ptr[my_id]], my_list_size, sizeof(int), compare_int);
    
    // Wait for all threads to finish sorting their local lists
    pthread_barrier_wait(&barrier);
    
    if (DEBUG && my_id == 0) print_list(list, list_size);
    
    // Parallel merge sort iterations
    for (level = 0; level < q; level++) {
        ...
        // Wait for all threads to finish scattering
        pthread_barrier_wait(&barrier);
...
int main(int argc, char *argv[]) {
    ...
    for (my_id = 0; my_id < num_threads; my_id++) {
        thread_args[my_id].thread_id = my_id;
        thread_args[my_id].q = q;
    }
    ...
    // Create threads
    for (my_id = 0; my_id < num_threads; my_id++) {
        pthread_create(&threads[my_id], NULL, thread_sort, &thread_args[my_id]);
    }
